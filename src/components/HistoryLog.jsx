      ...
      <div className="mt-6 space-y-3 stagger-children">
        {loading ? (
          ...
          filteredWorkouts.map((workout) => (
            <div key={workout.id} className="group flex flex-col gap-3 rounded-card glass-card p-5 relative overflow-hidden">
              ...
              {workout.sets_data && workout.sets_data.length > 0 ? (
                ...
                <div className="mt-1 inline-block font-mono text-base font-bold text-text-main bg-app-bg px-4 py-2 rounded-xl border border-glass-border w-fit">
                  {workout.sets} &times; {workout.reps} @ {workout.weight_kg} kg
                </div>
              )}
              
              {workout.custom_notes ? (
                <div className="mt-1 rounded-xl bg-card-elevated/50 px-4 py-3 border-l-2 border-accent-lime">
